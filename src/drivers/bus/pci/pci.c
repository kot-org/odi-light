...
static void pci_write32(uint32_t addr, uint32_t data) {
    addr &= ~(0b11);
    /* write address */
    odi_dep_outb(PCI_CONFIG_ADDR, addr);
    /* write data */
    odi_dep_outb(PCI_CONFIG_DATA, data);
}
...
void send_configuration_space_pci(pci_device_t* device){
    ...
    // update back buffer
    odi_dep_memcpy((void*)back_buffer, (void*)front_buffer, PCI_CONFIGURATION_SPACE_SIZE);
}
...
static void get_device(pci_device_list_info_t* pci_device_list, uint16_t bus, uint16_t device, uint16_t func){
    ...
    pci_device_t* device_info = (pci_device_t*)odi_dep_malloc(sizeof(pci_device_t));
    device_info->is_pcie = false;
    device_info->address = address; // this is not pci device, it's pcie device
    device_info->configuration_space = odi_dep_malloc(PCI_CONFIGURATION_SPACE_SIZE);
    device_info->configuration_space_back = odi_dep_malloc(PCI_CONFIGURATION_SPACE_SIZE);
    ...
    odi_dep_memcpy(device_info->configuration_space, device_info->configuration_space_back, PCI_CONFIGURATION_SPACE_SIZE);
